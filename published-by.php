...
{    
    ...
    */
    public function ma_admin_menu()
    {
        ...
        # Adding new report based on author name
        add_menu_page('Download author Report', 'Download author Report', 'manage_options', 'download_author_report', array($this, 'ma_download_author_report'));
    ...
    */
    public function ma_download_report()
    {
        ...
        if(isset($_POST['submit_date'])){
            ...
            $ma_csv_count = $this->ma_csvFile_count();
            echo $ma_csv;
            echo $ma_csv_count;
    ...
    /**
    * Download Author report
    */   
    public function ma_download_author_report()
    {
        echo '<div class="wrap">';
        echo '<div id="icon-tools" class="icon32">
        </div>';
        ...
        $ma_form .= '<h3>Select Username</h3>';
        
        // Get Author List
        $ma_authors_list = get_users();
        
        $ma_form .= '<select name="author_id">';
        
        foreach( $ma_authors_list as $ma_auhor){
            $ma_form .= "<option value=\"$ma_auhor->id\">$ma_auhor->display_name </option>";   
        }
        ...
        if(isset($_POST['submit_date'])){
            ...
            // Print report Download Link 
            $ma_csv = $this->ma_author_csvFile($ma_start_date, $ma_end_date, $ma_auhor_id);
            echo $ma_csv;
        }
    }
    ...
    */
    public function ma_csvFile($ma_start_date, $ma_end_date) {
        ...
        // Create CSV Header
        $ma_columns = array('id', 'title', 'views', 'date', 'url', 'displayed author', 'published by');  
        ...
        if ( $ma_query->have_posts() ) {
            ...
            while ( $ma_query->have_posts() ) {
                ...
                fputcsv( 
                    $ma_outstream, 
                    [ get_the_ID(), get_the_title(), reset(get_post_meta(get_the_ID(),'tie_views')), get_the_date(), get_permalink(), $ma_author_name, $ma_publisher_name ] 
                );
    ...
    /**
    * Exporting author data to CSV
    */
    public function ma_author_csvFile($ma_start_date, $ma_end_date, $ma_author_id) {
